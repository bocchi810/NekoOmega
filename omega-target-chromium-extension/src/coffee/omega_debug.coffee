...
window.OmegaDebug =
  ...
  downloadLog: ->
    idbKeyval.entries(logStore).then((entries) ->
      zip = new JSZip()
      zipFolder = zip.folder('NekoOmega')
      ...
      filename = "NekoOmegaLog_#{Date.now()}.zip"
  ...
  reportIssue: ->
    idbKeyval.get('lastError', logStore).then((lastError) ->
      url = 'https://github.com/bocchi810/NekoOmega/issues/new?title=&body='
      finalUrl = url
      try
